...
var FitController = (function () {
    function FitController($http) {
        this.loadData($http);
    }
    FitController.prototype.loadData = function ($http) {
        var that = this;
        $http({ method: 'GET', url: '/stories/HomePage' }).success(function (data, status, headers, config) {
...
(function () {
    angular.module('chaas').directive('chaasFixture', function chaasFixtureDirective() {
        return {
            restrict: 'E',
            controller: [
                'CONFIG', '$http', '$element', function chaasFixtureLink(CONFIG, $http, $element) {
                    CONFIG.then(function () {
                        var path = CONFIG.fixtures[0];
